...
const PatientDataView = ({ patientData }) => {
  ...
  // Handle update click - route based on patient status
  const handleUpdateClick = () => {
    // Route based on patient status
    if (patientData.status === "Admitted") {
      navigate(`/patients/inpatient/edit/${patientData.id}`);
    } else {
      navigate(`/patients/outpatient/edit/${patientData.id}`);
    }
  };
