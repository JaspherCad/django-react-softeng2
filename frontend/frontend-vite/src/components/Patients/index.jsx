...
import { addPatientAPI, editPatientAPI, listOfPatientAPI, editPatientINCLUDINGCASENUMBERAPI } from '../../api/axios';
...
const Patients = () => {
  ...
  const handleEditExistingPatient = async (id, updatedFOrmData) => {
    try {
      console.log('Editing existing patient with new case number:', id)
      const response = await editPatientINCLUDINGCASENUMBERAPI(id, updatedFOrmData)
      //fetch patient
      await fetchPatient()
    } catch (error) {
      console.error('Error editing existing patient:', error)
  ...
  return (
    <Routes>
      ...
      {/* Routes for existing patients - generates NEW case number */}
      <Route path="outpatient/existing/:id" element={<OutPatientForm onSubmit={handleEditExistingPatient} loading={loading} errorMsg={errorMsg} />} />
      <Route path="inpatient/existing/:id" element={<InPatientForm onSubmit={handleEditExistingPatient} loading={loading} errorMsg={errorMsg} />} />

      {/* Routes for editing patients - keeps SAME case number */}
