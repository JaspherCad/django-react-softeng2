...
const BillingItemsModal = ({ closeModal, billingItem, loading, error, billId, billCode,setReloadCount }) => {
  ...
  const handleCreateOrUpdate = async () => {
    ...
    try {
      ...
      console.error('Error submitting billing item:', err.response?.data.error || err.message);
      // Display error message from the API or a generic message
      setErrorMessage(err.response?.data.error || 'Failed to save billing item. Please try again.');
    }
        ...
        <div style={{ display: 'flex', gap: '10px', alignItems: 'flex-start' }}>
          <div style={{ flex: 3 }}>
            ...
            <div className={styles.inputGroup}>
              ...
              <SearchBar
                placeholder={"Seach Service by name or code"}
