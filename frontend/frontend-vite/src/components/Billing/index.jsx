...
import {
  getBillingByID,
  SearchBillingsApi,
  SearchBillingsApiExcludingDischarged,
} from "../../api/axios";
...
const Billing = () => {
    ...
    <>
      <div className={styles.billingContainer}>
        <div className={styles.leftPanel}>
          <button onClick={handleAddBilling}>Add +</button>
        ...
        <div className={styles.rightPanel}>
          ...
          {selectedItem ? (
            <div className={styles.patientBox}>
              ...
              <p>
                <strong>Name:</strong> {selectedItem.patient.name}
              </p>
              <p>
                <strong>Status:</strong> {selectedItem.patient.status}
              </p>
              <p>
                <strong>Admission Date:</strong>{" "}
                {selectedItem.patient.admission_date}
              </p>
              <p>
                <strong>Phone:</strong> {selectedItem.patient.phone}
              </p>
              <p>
                <strong>Email:</strong> {selectedItem.patient.email}
              </p>

              <h3>Billing Details</h3>
              <p>
                <strong>Billing ID:</strong> {selectedItem.id}
              </p>
              <p>
                <strong>Status:</strong> {selectedItem.status}
              </p>
              <p>
                <strong>Total Due:</strong> ${selectedItem.total_due}
              </p>
              <p>
              ...
              <ul>
                {selectedItem.billing_items.map((item) => (
                  <li key={item.id}>
                    Service Availed: {item.service_availed} | Quantity:{" "}
            ...
            <p className={styles.noSelection}>
              Select a billing item to view details
            </p>
          )}
        </div>
      </div>
      ...
      <AddBillingModalV2
        show={isAddBillingModalOpen}
        onClose={handleCloseAddBillingModal}
        setModalOpen={setIsAddBillingModalOpen}
      />
    </>
  );
  ...
  return (
    <Routes>
      ...
      <Route
        path="billing_items_of_billing/:billId"
        element={<BillingItemsOfThatBill />}
      />
