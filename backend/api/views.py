    ...
    if request.method == "GET":
        try:
            ...
            if hasattr(request.user, 'role') and request.user.role == 'Doctor':
                ...
                # Get patient IDs where doctor was attending physician in history
                ...
                # Filter patients based on these IDs
                list_of_patient_query = list_of_patient_query.filter(id__in=all_patient_ids)
...
@api_view(['PUT']) #UPDATES the patient NOT THE CASE CODE
@permission_classes([IsAdmin | IsDoctor | IsNurse | IsReceptionist])
def patient_update(request, pk):
    if request.method == "PUT":
        try:
            data = request.data.copy()
            data.pop('case_number', None)  # stops case_number update
            data.pop('hospital_case_number', None)  # stops hospital_case_number update
...
def patient_update_new_case_number(request, pk):
    if request.method == "PUT":
        try:
            ...
            if serializer.is_valid():
                # Save the patient and force hospital_case_number to remain the same
                updated_patient = serializer.save(_history_user=request.user)
                # Ensure hospital_case_number stays the same as before
                if updated_patient.hospital_case_number != original_hospital_case_number:
                    updated_patient.hospital_case_number = original_hospital_case_number
                    updated_patient.save(update_fields=['hospital_case_number'])
                
                log_action(
                    user=request.user,
                    action="UPDATE_WITH_NEW_CASE_NUMBER",
                    details={
                        "message": "Updated patient details with new case number",
                        "patient_id": updated_patient.id,
                        "patient_name": updated_patient.name,
                        "updated_fields": list(data.keys()),
                        "old_case_number": request.data.get('case_number'),
                        "new_case_number": updated_patient.case_number,
                        "hospital_case_number": updated_patient.hospital_case_number
                    }
                )
                return Response(serializer.data, status=status.HTTP_200_OK)
...
def create_bill_item(request, pk):
    ...
    if billing.status == 'Paid':
        return Response(
            {"error": "Cannot add new bill item. This billing is already marked as paid."},
            status=status.HTTP_400_BAD_REQUEST
        )
...
def upload_user_image(request, user_id):
    ...
    """
    print(f"[DEBUG] Upload user image - User ID: {user_id}")
    ...
    upload_list = request.FILES.getlist('file')
    if not upload_list:
        print(f"[DEBUG] No files found under 'file' key. Available keys: {list(request.FILES.keys())}")
        return Response(
