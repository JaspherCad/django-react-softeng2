...
def generate_case_number():
    """Generate case number in format CN-YYYYMMDD-XXX"""
    ...
    # This will be called during model creation, so we need to be careful about imports
    # We'll use a raw query to avoid circular import issues
    
    try:
        with connection.cursor() as cursor:
            cursor.execute(
                "SELECT case_number FROM api_patient WHERE case_number LIKE %s ORDER BY case_number DESC LIMIT 1",
                [f"{prefix}%"]
            )
        ...
        if not result:
            ...
            try:
                # Extract sequence number from the last case number
                last_case = result[0]
                seq_part = last_case.split('-')[-1]
                sequence = int(seq_part) + 1
            except (ValueError, IndexError):
                sequence = 1
    except Exception:
        # Fallback if database query fails (e.g., during migrations)
        import random
        sequence = random.randint(1, 999)
    
    return f"CN-{date_str}-{sequence:03d}"
